...
public class CustomReporterBuildSummary extends CustomReporterListener {
	...
	public void testCaseNo() {
		...
		m_out.println(
				"<table width='450px' height='200px' border='5' align='left'><tbody><tr colspan='2'><td bgcolor='#0088cc' colspan='2'><h1><center><font color='white'>Build Summary</font></center></h1></td></tr><tr bgcolor='#b1edc0'><td><b>"
						...
						+ "<tr><td><b><a href='CustomReporterTestFailureDetails.html' target='iframepass'>Failed Test Cases</a></b></td><td> <center><b>"
						+ (failedcount)
						+ "</b></center></td></tr> 	 <tr bgcolor='#fcf77a'><td><b><a href='CustomReporterTestSkippedDetails.html' target='iframepass' >Skipped Test cases</b>   </td><td><center><b> "
						+ skipped
						+ "</b></center> </td></tr><tr bgcolor='skyblue'><td> <b><a href='CustomReporterAllTestDetails.html' target='iframepass' >Total Test Cases </a></b>  </td><td> <center><b>"
						+ qty_tests + "</b></center></td></tr></tbody></table>");

		m_out.println(
				"<div><iframe name='iframepass' src='CustomReporterTestPassedDetails.html' align='top' height='1000px' width='73%' hspace='10' vspace='10'></div>");
		m_out.println("</center>");
	}
		...
		if (tests.getAllResults().size() > 0) {
			...
			for (ITestNGMethod method : getMethodSet(tests, suite)) {
				...
				if (!checkTestCases(testname) && !isPassed(testname)) {
					...
					{
						String id = (m_testIndex == null ? null : "test" + Integer.toString(m_testIndex));
						num = m_testIndex;
	...
	private void resultSummarypassed(ISuite suite, IResultMap tests, String testname, String style, String details) { // 5.4.1
		...
		if (tests.getAllResults().size() > 0) {
			...
			for (ITestNGMethod method : getMethodSet(tests, suite)) {
				...
				if (!checkTestCasespassed(testname) && (isPassed(testname) || isskipped(testname))) {
					...
					m_out.println(
							"><td width='25%' style=\"font-size:14px; font-family:Times New Roman;background-color:##aae2c4\">"
									+ testname + "</td>");
	...
	private void startResultSummaryTable(String style) { // 5.2.1
		...
		m_out.println("<tr><td bgcolor='white' colspan='4'> <table border='0' width='100%' bgcolor='#e6f7ff'><tr>"
				...
				+ "<center><img width='150px' src='https://www.narolainfotech.com/images/logo.png'/></center>"
				+ "</td><td ><center><font color='#008bcc'><b><h1>Automation Test Cases Analysis</h1></b></font></center></td> "
				+ "<td width='25%' bgcolor='white'>" + "<center><img width='30px' src='https://" + env
				+ ".thegreenrfp.com/assets/images/logos/rfp-icon.png'/>  <img width='150px' src='https://" + env
				+ ".thegreenrfp.com/assets/images/logos/rfp-text.png'/></center></td> " + "</tr></table> </td></tr>");
		...
		m_out.println("<tr><td colspan='4'>Overall test suite completion : <b>" + Time
				...
				+ "<t></t> </b><br/>OS: <b>" + System.getProperty("os.name") + "</b></td></tr>");
		...
		for (ITestResult result : tests.getAllResults()) {
			...
			for (String line : msgs) {
				if (g == 0) {
					...
					m_out.println("<a href=\"#hide" + num
							+ "\" data-toggle=\"tooltip\" title=\"Click here to see list of steps\" class=\"hide\" id=\"hide"
							+ num + "\">+</a>");
					m_out.println("<a href=\"#show" + num + "\" class=\"show\" id=\"show" + num
							+ "\">-</a></b> &nbsp;&nbsp;Click here to see list of steps</br></br>");
	...
	protected void endHtml(PrintWriter out) {
		...
		out.println(
				"<tr bgcolor='SkyBlue'><td align='right' colspan='4'><center><b><i>Report customized by Narola Infotech </i><b><center></center></b></b></center></td></tr>");
