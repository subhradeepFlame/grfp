...
public class CustomReporterTestskipDetails extends CustomReporterListener {
	...
	protected PrintWriter createWriter(String outdir) throws IOException { // 2.1
		...
		return new PrintWriter(
				new BufferedWriter(new FileWriter(new File(outdir, "CustomReporterTestSkippedDetails" + ".html"))));
	}
	...
	protected void generateMethodSummaryReport(List<ISuite> suites) { // 5.1
		...
		for (ISuite suite : suites) {
			...
			for (ISuiteResult r2 : r.values()) {
				...
				resultSummary(suite, testContext.getSkippedTests(), testName, "failed", "");
		...
		if (tests.getAllResults().size() > 0) {
			...
			for (ITestNGMethod method : getMethodSet(tests, suite)) {
				...
				if (!checkTestCases(testname) && !isPassed(testname)) {
					...
					{
						String id = (m_testIndex == null ? null : "testskiptoggle" + Integer.toString(m_testIndex));
		...
		for (ITestResult result : tests.getAllResults()) {
			...
			for (String line : msgs) {
				if (g == 1) {
					...
					m_out.println("<a href=\"#hide" + toggle
							+ "\" data-toggle=\"tooltip\" title=\"Click here to see list of steps\" class=\"hide\" id=\"hide"
							+ toggle + "\">+</a>");
					m_out.println("<a href=\"#show" + toggle + "\" class=\"show\" id=\"show" + toggle
